<template>
  <span>
    <v-btn small color="primary" @click.stop="handlerOpenDialog">更新</v-btn>
    <v-dialog v-model="dialog" max-width="500" persistent scrollable>
      <v-card class="px-1">
        <v-card-title>
          <span class="headline">更新告警规则</span>
        </v-card-title>
        <v-divider></v-divider>
        <v-card-text>
          <v-container>
            <v-row>
              <v-col>
                <v-form v-model="valid" ref="updateRuleForm">
                  <v-text-field
                    v-model="obj.name"
        ...
        <v-card-actions>
          <v-layout row justify-space-around>
            <v-flex xs5>
              <v-btn color="primary" block @click="handlerUpdateLogRule">
                保存
              </v-btn>
            </v-flex>
            <v-flex xs5>
...
<script>
import { updateLogRule, listLogGroup } from '@/api'
...
export default {
  ...
  methods: {
    ...
    async handlerUpdateLogRule() {
      try {
        if (this.$refs.updateRuleForm.validate()) {
          const res = await updateLogRule(Object.assign(this.obj))
          if (res.status === 201) {
            this.$emit('refresh')
            this.$store.commit('showSnackBar', {
              text: 'Success: 更新成功',
    ...
    handlerCloseDialog() {
      this.dialog = false
      this.$refs.updateRuleForm.reset()
    },
    async handlerOpenDialog() {
      await this.listLogGroup()
      this.obj = copy(this.item)
      const groups = []
      this.obj.groups.forEach((group) => {
        Object.assign(group, {
          group_name: group.log_user_group.group_name,
          users: group.log_user_group.users,
        })
        delete group.log_user_group
        groups.push(group)
      })
      this.obj.groups = groups
      this.labeled = this.obj.labels.length > 0
      this.handlerShowGroupUsers()
      this.dialog = true
    },
  },
}
