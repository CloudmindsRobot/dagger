    ...
    <v-card class="px-1">
      ...
      <v-card-text>
        ...
        >
          <v-list-item tag="span">
            <v-list-item-content>
              <v-list-item-title>规则: {{ item.name }}</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </v-list>
      ...
      <v-card-actions>
        <v-row justify="space-around">
          <v-col cols="6" md="3">
            <v-btn color="error" @click="deleteLogRule(item)" block
              >确定删除</v-btn
            >
          </v-col>
          <v-col cols="6" md="3">
...
export default {
  ...
  methods: {
    async deleteLogRule() {
      this.loading = true
      try {
        const res = await deleteLogRule({ id: this.item.id })
        if (res.status === 204) {
          this.$emit('refresh')
        } else {
